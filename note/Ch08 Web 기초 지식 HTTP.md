...
- 302는 너가 요청한게 당분간, 임시적으로 다른 곳으로 갔기 때문에 여기를 이용하라는 식으로 나타낼 수 있다

- 303은 302와 유사한데 GET 요청에서만 사용 가능

- 클라이언트가 POST 요청했을 때, POST에 한해서만 다른 곳으로 옮겨갔다면 307, 308을 나타낸다

- 사용자는 POST 요청에만 다른 곳으로 옮겨갔기 때문에 POST 요청을 이용해서 옮겨간 URL로 다시 요청해야겠다고 이해할 수 있다

- 즉, 일반적인 302 (Found) 리다이렉트는 POST 요청을 보내도 브라우저가 새 URL로 이동할 때 멋대로 GET으로 바꿔버리는 고질적인 문제가 있었습니다. 이를 해결하기 위해 나온 것이 307과 308
...
- status code를 상세하게 하는게 좋은가요? 간략화하는게 좋은가요?

- status code에도 여러가지 정보들이 담기기 때문에 해커입장에서는 상세하면 할수록 파고들 여지가 많다고 들었어요.

- 그렇다면....실제 현업에서는 200, 400으로 간단하게 표현하는 경우가 많은지 아니면 그래도 상세하게 201, 401, 404 등등을 자세하게 표현하는게 좋은건지 궁금합니다.

- 솔직히 공부하기 전에는 404만 본적이 있고, 다른 코드들에 큰 의미를 가진적이 없었는데, 공부를 하고나서는 상세하게 표현하면 좋겠구나 라는 생각을 했었거든요.

- 그런데 또 다시 생각해보면 사용자가 status code를 알아야할 필요가 있을까 싶기도 하구요.

### A.

- Response 바디에 너무 상세한 (비밀번호가 틀렸다! 아이디가 중복이다!) 이런 메시지는 가급적 보내지 않는것이 좋지만 -> 무조건 401

- 요청 자체가 유효하지 않는것인지 400, 허용되지 않은 사용자인지 401

- 이런것들은 올바른 status code를 사용해야 합니다 :)

1. 브라우저 (클라이언트)가 code에 따라서 표준적으로 하는 행위가 있을 수 있음

2. 다른 클라이언트 (모바일 앱)와 의사소통 하기 좋음
