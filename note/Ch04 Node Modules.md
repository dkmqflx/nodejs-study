...
- 앞선 질문과 동일하게 콜백함수를 이용하는 rename과 promise를 이용하는 rename을 차례대로 실행하면 어느것이 먼저 실행되는지 알 수 없다고 하신거에 대한 질문입니다! 제가 자료구조를 배웠을 때 Queue는 FIFO구조이기 때문에 먼저 들어온것이 제일 먼저 나오는걸로 알고있습니다. 그럼 한줄씩 읽으며 내려갈 때

- 콜백함수를 이용하는 rename이 먼저 TaskQueue안에 들어가게되고 promise를 이용한 rename이 두번째 들어가면서 콜백함수를 이용한 rename이 먼저 실행되는 것이 아닌가요?
